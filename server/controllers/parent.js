...
module.exports = {
        ...
        .then((data) => {
            if (!data) {
                ...
                return res.status(200).json({ 
                    success: true, message: 'Succesfully created parent', parent: data 
                }).end();
            }
        })
        .catch((err) => {
    ...
    getAllParents: function (req, res, next) {
        Parents.findAll({})
		.then((parents) => {
			if(parents === null) {
    ...
    deleteParent: function(req, res, next){
        var id = req.params.id;
        if (!id) {
            return res.status(400).json({ success: false, message: "Parameter is missing" }).end();
        }
        ...
        .then(function() {
            return res.status(200).send({"msg": "Succesfully deleted parent."}).end();
        })
        .catch(function(err) {
            console.log(err);
    ...
    updateParent: function (req, res, next) {
		...
		.then((data) => {
			return res.status(200).json({ success: true, message: 'Succesfully updated parent', parent: data }).end();
		})
		.catch((error) => {
			console.log(err);
			return res.status(500).json({ success: false, message: "Internal Server Error" }).end();
